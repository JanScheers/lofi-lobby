 ...
 *   - public/downloads/ (game zip copies)
 *   - public/images/ (game thumbnails)
 *   - src/data/games.json → reset to { "games": [] }
...
const PATHS_TO_REMOVE = [
  ...
  path.join(ROOT_DIR, 'public', 'downloads'),
...
function main() {
  ...
  if (fs.existsSync(METADATA_FILE)) {
    if (dryRun) {
      ...
      fs.writeFileSync(METADATA_FILE, JSON.stringify(EMPTY_METADATA, null, 2) + '\n');
